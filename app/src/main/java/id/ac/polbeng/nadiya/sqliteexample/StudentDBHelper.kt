...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
    ...
    fun searchStudentByNIM(nim: String): ArrayList<StudentModel> {
        ...
        val cursor: Cursor?
        try {
            cursor = db.rawQuery("select * from " + DBContract.UserEntry.TABLE_NAME + " WHERE " + DBContract.UserEntry.COLUMN_NIM + "='" + nim + "'", null)
    ...
    fun searchStudentByName(name: String): ArrayList<StudentModel> {
        ...
        val cursor: Cursor?
        try {
            cursor = db.rawQuery("select * from " + DBContract.UserEntry.TABLE_NAME + " WHERE " + DBContract.UserEntry.COLUMN_NAME + " LIKE \"%"+name+"%\"", null)
        } catch (e: SQLiteException) {
            // if table not yet present, create it
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
